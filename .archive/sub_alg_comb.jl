...
function Aopt(y::AbstractVector, d::AbstractVector, r::Int, θplt::AbstractVector, Mdd_plt::AbstractMatrix, rplt::Int;
    ...
    ddopt = ϕdd(y, d, θopt, findall(samp), PI)/z_scale

    ropt = suminv(PI, findall(samp)) * sum(samp) / length(d) / z_scale
    
    return combine(ddopt, θopt, ropt, Mdd_plt[1,1] * rplt, θplt, rplt), ite
...
function Aopt(y::AbstractVector, d::AbstractVector, r::Int, plt_itr;
    ...
    return combine(ddopt, θopt, ropt, Mdd_plt[1,1] * length(plt_itr), θplt, length(plt_itr)), ite
end
...
function Aopt(y::AbstractVector, d::AbstractVector, r::Int, θplt::AbstractVector, Mdd_plt::AbstractMatrix, rplt::Int,α::Float64;
    ...
    ddopt = ϕdd(y, d, θopt, findall(samp), PI)

    ropt = suminv(PI, findall(samp)) * sum(samp) / length(d)
...
function iboss(y::AbstractVector, d::AbstractVector, r::Int, plt_itr;
    ...
    ddiboss = ϕdd(y, d, θiboss, findall(δ))
    ddplt = ϕdd(y, d, θiboss, plt_itr)
    
    return combine(ddiboss, θiboss, r, ddplt, θplt, length(plt_itr)), ite
end
